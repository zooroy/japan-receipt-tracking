...
- **收據掃描**：點「+」直接觸發原生相機／相簿選擇器，支援一次選取多張照片批次上傳
- **AI 辨識**：由 Gemini AI 自動擷取店名、金額、品項明細、消費分類、稅別，並驗證是否為日本收據
- **多旅程管理**：依旅程分類管理收據，可在旅程頁切換當前旅程
- **消費儀表板**：總支出概覽、每日消費折線圖、分類圓餅圖、稅別統計
- **日幣換算**：即時取得 JPY → TWD 匯率，每筆收據同時記錄日幣與台幣金額
- **重複偵測**：透過圖片 hash 防止重複上傳同一張收據
- **單一密碼認證**：簡易密碼保護，JWT 維持登入狀態
- **PWA 支援**：可加入主畫面，提供接近原生 App 的體驗
...
2. **建立旅程**：前往「旅程」頁，輸入旅程名稱與日期並設為當前旅程
3. **新增收據**：點底部導航列中央的「+」，選取一張或多張收據照片
4. **AI 辨識**：系統自動辨識並填入店名、金額、分類等欄位，可手動修正後儲存
5. **批次上傳**：選取多張照片時，系統依序顯示每張的確認表單，逐一儲存
6. **查看儀表板**：首頁顯示當前旅程的消費總覽、圖表與最近收據
7. **旅程切換**：前往「旅程」頁即可切換或建立新旅程
